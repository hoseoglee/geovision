...
function GeofenceCreateForm({ onFinish, onCancel }: {
  ...
  return (
    <div className="mt-2 p-2 bg-gray-800/60 rounded border border-gray-700/50 space-y-2">
      <div className="flex items-center gap-2">
        <span className="text-gray-500 text-[9px] w-10 shrink-0">NAME</span>
        <input type="text" value={name} onChange={(e) => setName(e.target.value)}
          className="flex-1 bg-gray-900/80 border border-gray-700/50 rounded px-1.5 py-0.5 text-[10px] text-gray-300 font-mono outline-none focus:border-cyan-500/50"
          autoFocus
          onKeyDown={(e) => {
      ...
      <div className="flex items-center gap-2">
        <span className="text-gray-500 text-[9px] w-10 shrink-0">COLOR</span>
        <div className="flex gap-1">
          {COLORS.map((c) => (
            <button key={c} onClick={() => setColor(c)}
              className={`w-4 h-4 rounded-sm border transition-all ${color === c ? 'border-white scale-110' : 'border-gray-600/50 hover:border-gray-400'}`}
      ...
      <div className="flex items-center gap-2">
        <span className="text-gray-500 text-[9px] w-10 shrink-0">TRACK</span>
          ...
          {LAYER_OPTIONS.map((opt) => (
            <button key={opt.value} onClick={() => toggleLayer(opt.value)}
              className={`text-[8px] font-mono px-1.5 py-0.5 rounded border transition-colors ${
                layers.includes(opt.value) ? 'border-cyan-500/50 text-cyan-400 bg-cyan-900/20' : 'border-gray-700/50 text-gray-600 hover:text-gray-400'
              }`}>{opt.label}</button>
          ))}
        </div>
      </div>
      <div className="flex justify-end gap-1.5 pt-1">
        <button onClick={onCancel} className="text-[9px] font-mono px-2 py-0.5 rounded border border-gray-700/50 text-gray-500 hover:text-gray-300 transition-colors">CANCEL</button>
  ...
  return (
    <div className={`flex items-center gap-1.5 py-1 group ${!gf.enabled ? 'opacity-40' : ''}`}>
      <button onClick={handleNavigate} className="flex-1 min-w-0 flex items-center gap-1.5 text-left hover:text-cyan-400 transition-colors" title="Navigate">
        ...
        <span className="text-gray-300 text-[10px] font-mono truncate">{gf.name}</span>
      </button>
      ...
      <button onClick={() => toggleGeofence(gf.id)}
        className={`text-[8px] font-mono px-1 py-0.5 rounded border transition-colors shrink-0 ${gf.enabled ? 'border-green-500/40 text-green-400 hover:bg-green-900/30' : 'border-gray-600/40 text-gray-600 hover:bg-gray-800/30'}`}>
        {gf.enabled ? 'ON' : 'OFF'}
      </button>
      <button onClick={() => removeGeofence(gf.id)} className="text-gray-600 hover:text-red-400 text-[10px] shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" title="Remove">{'\u2715'}</button>
    </div>
  );
}
...
function DrawingStatus() {
  ...
  return (
    <div className="mt-2 p-1.5 bg-cyan-950/30 border border-cyan-500/30 rounded text-[9px] font-mono">
      ...
      {drawingMode === 'polygon' && <p className="text-gray-600 text-[8px] mt-0.5">Click map to add vertices. Press FINISH when done.</p>}
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-gray-400 text-[10px] font-bold tracking-widest">GEOFENCE MONITOR</h3>
        {geofences.length > 0 && <span className="bg-gray-700/60 text-gray-400 text-[9px] px-1.5 py-0.5 rounded-full font-mono">{geofences.length}</span>}
      </div>
      ...
      {geofences.length === 0 && !drawingMode && <p className="text-gray-600 text-[8px] font-mono mt-1">No geofences. Draw one to start monitoring.</p>}
    </div>
  );
}
...
function EventRow({ event }: { event: GeofenceEvent }) {
  ...
  return (
    <div className="px-3 py-2 border-b border-gray-800/50 cursor-pointer hover:bg-gray-800/40 transition-colors"
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-gray-600 text-[9px] font-mono">{timeAgo(event.timestamp)}</span>
      </div>
      <p className="text-gray-500 text-[9px] font-mono mt-0.5">{event.entityLayer} {event.entityId}</p>
    </div>
  );
}
    ...
    return (
      <button onClick={() => setExpanded(true)}
        className={`fixed bottom-8 right-4 z-50 flex items-center gap-1.5 px-2.5 py-1.5 rounded font-mono text-xs backdrop-blur-sm transition-all border ${
          hasEnter ? 'bg-red-900/80 border-red-500/60 text-red-400 animate-pulse' : 'bg-gray-800/60 border-gray-600/40 text-gray-400'
        }`}>
        <span className="text-[10px]">{'\u25CE'}</span>
        <span className="font-bold tracking-wider">GEOFENCE</span>
        {recentCount > 0 && <span className={`text-[9px] px-1 py-0.5 rounded-full font-bold ${hasEnter ? 'bg-red-600/80 text-white' : 'bg-gray-600/80 text-white'}`}>{recentCount}</span>}
      </button>
    );
  }

  return (
    <div className="fixed bottom-8 right-4 z-50 w-[380px] max-h-[400px] flex flex-col bg-gray-900/95 backdrop-blur-md border border-gray-700/50 rounded shadow-2xl font-mono pointer-events-auto">
      <div className="flex justify-between items-center px-3 py-2 border-b border-gray-700/40">
        <div className="flex items-center gap-2">
          <span className="text-cyan-400 text-[10px]">{'\u25CE'}</span>
          <span className="text-gray-300 text-xs font-bold tracking-widest">GEOFENCE EVENTS</span>
          {events.length > 0 && <span className="bg-gray-700/60 text-gray-400 text-[9px] px-1.5 py-0.5 rounded-full font-bold">{events.length}</span>}
        </div>
        <div className="flex items-center gap-2">
          <button onClick={clearEvents} className="text-gray-500 hover:text-gray-300 text-[10px] border border-gray-700/40 px-2 py-0.5 rounded">CLEAR</button>
          <button onClick={() => setExpanded(false)} className="text-gray-500 hover:text-gray-300 text-xs">{'\u2715'}</button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700">
        {events.length === 0 ? (
          <div className="text-gray-600 text-xs text-center py-8 font-mono">NO GEOFENCE EVENTS</div>
